...
export class BetaRedeemedEmailProcessor {
  ...
  async handleBetaRedeemedEmail(job: Job<BetaRedeemedEmailJobData>): Promise<void> {
    const { userId, email, name, betaAccessUntil } = job.data;

    this.logger.log(`Processing beta_redeemed_email job ${job.id} for user ${userId}`);

    const templateId = this.configService.get<number>('BREVO_TEMPLATE_ID_BETA_REDEEMED_WELCOME');
    if (!templateId) {
      this.logger.warn(`BREVO_TEMPLATE_ID_BETA_REDEEMED_WELCOME not configured — skipping email for user ${userId}`);
      return;
    }
    ...
    try {
      ...
      this.logger.log(`Beta redeemed welcome email sent to ${email} (user: ${userId})`);
    } catch (error) {
      this.logger.error(`Failed to send beta redeemed welcome email to ${email} (user: ${userId})`, {
