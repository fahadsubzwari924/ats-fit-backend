...
export class SubscriptionPlanService {
   ...
   * });
   */
  async findAll(queryOptions: ISubscriptionPlanQueryOptions = {}): Promise<SubscriptionPlan[]> {
    try {
      // Validate input parameters
      SubscriptionPlanQueryBuilder.validateQueryOptions(queryOptions);
      
      // Build TypeORM query options with defaults applied
      const findOptions = SubscriptionPlanQueryBuilder.buildFindOptions(queryOptions);
   ...
   * });
   */
  async findAllIncludeInactive(queryOptions: ISubscriptionPlanQueryOptions = {}): Promise<SubscriptionPlan[]> {
    try {
      ...
      const findOptions = SubscriptionPlanQueryBuilder.buildFindOptions(modifiedOptions);
      
      // Remove is_active from where clause if it exists (to include inactive plans)
      if (findOptions.where && typeof findOptions.where === 'object' && 'is_active' in findOptions.where) {
