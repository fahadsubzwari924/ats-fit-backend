...
export class AtsMatchService {
  ...
  async calculateAtsScore(
    ...
    try {
      ...
      const cacheKey = this.generateCacheKey(jobDescription, resumeFile);
      ...
      // 4. Perform premium LLM-based ATS evaluation with timeout
  ...
  private generateCacheKey(
    ...
    // Create a hash of job description and file content for caching
    const jobHash = this.simpleHash(jobDescription);
    const fileHash = this.simpleHash(resumeFile.buffer.toString('base64'));
    return `ats_${jobHash}_${fileHash}`;
  }
  ...
  private async performPremiumAtsEvaluationWithTimeout(
    ...
    try {
      ...
      const evaluation = await Promise.race([
        this.performPremiumAtsEvaluation(resumeText, jobDescription),
        timeoutPromise,
      ]);
  ...
  private async performPremiumAtsEvaluation(
    ...
    const prompt = this.promptService.getPremiumAtsEvaluationPrompt(
      resumeText,
      jobDescription,
    );
    ...
    try {
      ...
      const standardEvaluation =
        (await this.aiService.evaluateResumeWithAtsCriteria(
          resumeText,
          jobDescription,
        )) as StandardAtsEvaluation;
