...
4. Emit a task list: each task must have **path**, **change**, **verify** (`npm run test` / manual check), and **Agency `subagent_type`** — the exact frontmatter `name` value you will pass to the Task tool when dispatching, pulled from `.ai/agents.md` or `.claude/agents/_index.json` `subagentType` field (e.g. `Backend Architect`, `Frontend Developer`, `API Tester`). Filename stems like `engineering-backend-architect` will not resolve.
