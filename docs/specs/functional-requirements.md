...
| **REQ-006** | P0 | User can obtain ATS-style score for JD + resume input; usage tracked; history available within plan window. | [05-ats-matching.md](./05-ats-matching.md) | `ats-match/` |
| **REQ-007** | P1 | User can create, list, filter, update, delete **job applications**; see aggregate stats; link optional ATS/generation refs. | [06-job-applications.md](./06-job-applications.md) | `job-application/` |
| **REQ-008** | P1 | User can list plans, start checkout, and—via **intended** webhook flow—activate subscription; payment history auditable. | [07-subscriptions-billing.md](./07-subscriptions-billing.md) | `subscription/` |
| **REQ-009** | P1 | System enforces per-feature usage / rate limits by plan; user can query usage for UI. | [08-rate-limits-and-usage.md](./08-rate-limits-and-usage.md) | `rate-limit/`, `FeatureType` |
| **REQ-010** | P0 | API errors are consistent; health checks exist; sensitive config is not embedded in specs. | [09-api-conventions.md](./09-api-conventions.md), [01-architecture.md](./01-architecture.md) | `health/`, exception layer |
...
- **US-3:** As a job seeker, I want to **see how well my resume matches** a JD so I know what to improve.
- **US-4:** As a job seeker, I want to **track applications** in one place so I do not lose follow-ups.
- **US-5:** As a job seeker, I want to **upgrade via subscription** so I get higher limits and premium features.
